    ...
    (
      ...
      setContext((c) => {
        ...
        signals.splice(i, 1, {
          ...
          s:
            target === "s1"
              ? [Number(e.target.value), signals[i].s[1]]
              : target === "s2"
              ? [signals[i].s[0], Number(e.target.value)]
              : signals[i].s,
        });
        return {
          ...c,
    ...
    return (
      ...
      {showClearConfirm && (
        <ConfirmOverlay onClick={() => setShowClearConfirm(false)}>
          <ConfirmPanel onClick={(e: React.MouseEvent) => e.stopPropagation()}>
            <ConfirmTitle>Discard recording?</ConfirmTitle>
            <ConfirmBody>
              All recorded signals will be permanently deleted. This cannot be undone.
            </ConfirmBody>
            <ConfirmButtons>
              <Button
                color="outline"
