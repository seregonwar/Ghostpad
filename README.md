...
| | |
|--|--|
| Payload loading via elfldr           | ✓ working |
| Virtual DualSense registration       | ✓ working |
| Assignment-screen auto-dismiss       | ✓ working (MBus `BindDeviceWithUserId`) |
| Face / D-pad / shoulder buttons      | ✓ working |
| Analog sticks                        | ✓ working |
| L2 / R2 analog triggers              | ✓ working (digital bit + analog byte) |
| PS / Touchpad / Options              | ✓ working |
| Autoclicker per button (latency tests) | ✓ working |
| Companion `hidDumper` payload        | ✓ working — reads live `ScePadData` via injection |
| Share / Create button                | Fucked |
