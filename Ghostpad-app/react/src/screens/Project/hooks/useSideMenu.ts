...
export const useSideMenu = (props: MenuProps) => {
    ...
    (
      ...
      if (action.type === "index")
        ...
        group.items[action.itemIndex] = {
          ...
          ...saveNameProps,
          data: {
            ...group.items[action.itemIndex].data,
            title: e.target.value,
          },
        };
        ...
        if (folder.items) {
          folder.items[action.itemIndex] = {
            ...
            ...saveNameProps,
            data: {
              ...folder.items[action.itemIndex].data,
              title: e.target.value,
            },
          };
  ...
  const addItem = React.useCallback(
    async (action: buttonActionProps): Promise<void> => {
      ...
      const id = uid();
      const itemIndex =
        action.type === "folder" && group.folders
          ? group.folders[action.folderIndex].items?.length || 0
          : group.items?.length || 0;
      const itemAction: itemActionProps | folderItemActionProps =
        action.type === "folder"
          ? {
              type: "folderItem",
              groupIndex: action.groupIndex,
              folderIndex: action.folderIndex,
              itemIndex,
            }
          : {
              type: "item",
      ...
      const rightButtons = itemRightButtons(itemAction);
      const newItem = {
        id: id,
        title: DEFAULT_COMMAND_TITLE,
        data: {
          id: id,
          title: DEFAULT_COMMAND_TITLE,
          path: `${action.groupIndex}/items/${itemIndex}`,
          signals: [],
        },
        _level: 0,
        _state: "default",
        ...
        _onClickOutside: (e: React.ChangeEvent<HTMLInputElement>) =>
          saveName(itemAction, e, rightButtons || []),
      };
    ...
    [itemRightButtons, props.saveTo, save, saveName]
  );
  ...
  const addFolder = React.useCallback(
    async (action: buttonActionProps) => {
      ...
      const newMenu = [...menuRef.current];
      const group = newMenu[action.groupIndex];
      const folderIndex = group.folders?.length || 0;
      const folderAction: folderActionProps = {
        type: "folder",
        groupIndex: action.groupIndex,
        folderIndex,
      };
      const rightButtons = folderRightButtons(folderAction);
      const newFolder = {
        id: uid(),
        ...
        _rightButtons: rightButtons,
        _onClickOutside: (e: React.ChangeEvent<HTMLInputElement>) =>
          saveName(folderAction, e, rightButtons || []),
      };
      if (action.type === "index")
        group.folders = group.folders?.concat(newFolder);
      await save(newMenu);
    },
    [folderRightButtons, props.saveTo, save, saveName]
  );
          ...
          command:
            item._state === "active"
              ? {
                  ...
                  id: item.id,
                  title: item.title,
                  signals: normalizeSignals(item.data?.signals),
                  path: path,
                }
              : {
                  id: "",
  ...
  const addNewGroup = React.useCallback(async (): Promise<void> => {
    try {
      ...
      const id = uid();
      const groupAction: indexActionProps = {
        type: "index",
        groupIndex: length,
      };
      const rightButtons = indexRightButtons(groupAction);
      const newGroup = {
        id: id,
        index: {
          ...createIndex(
          ...
          _isEditing: true,
          _onClickOutside: (e: React.ChangeEvent<HTMLInputElement>) =>
            saveName(groupAction, e, rightButtons || []),
        },
      };
  ...
  }, [createIndex, indexRightButtons, menu, save, saveName]);
